...
$(document).ready(function() {
	...
	// Desktop mode menu click events
	...
	$('body').on('click', '#goldM', function() {
		pageClicked("gold");
	});
	$('body').on('click', '#copperM', function() {
		pageClicked("copper");
	});
	$('body').on('click', '#silverM', function() {
		pageClicked("silver");
	});
	$('body').on('click', '#platinumM', function() {
		pageClicked("platinum");
	});

});
//Another way of processing events when page loads
jQuery(window).load(function() {
	$(".loader").delay(1000).fadeOut("slow"), $(".loader-img").delay(3e3).fadeOut()
});
...
function loadWatchlist() {
	...
	if (localStorage.gold == "yes") {
		empty = false;
		result += '<div class="col-xs-6 col-sm-3" style=""><a id="gold"   ><div class="flatty_shadow"><span class="color1">G</span></div><h3>Gold</h3> </a></div>';
	}
	if (localStorage.silver == "yes") {
		empty = false;
		result += '<div class="col-xs-6 col-sm-3" style=""><a id="silver"  ><div class="flatty_shadow"><span class="color2">S</span></div><h3>Silver</h3> </a></div>';
	}
	if (localStorage.platinum == "yes") {
		empty = false;
		result += '<div class="col-xs-6 col-sm-3" style=""><a id="platinum"  ><div class="flatty_shadow"><span class="color3">P</span></div><h3>Platinum</h3> </a></div>';
	}
	if (localStorage.copper == "yes") {
		empty = false;
		result += '<div class="col-xs-6 col-sm-3" style=""><a id="copper"  ><div class="flatty_shadow"><span class="color4">C</span></div><h3>Copper</h3> </a></div>';
	} else if (empty == true) {
		return "<p>You have no items on your watchlist for this session yet.</p>";
	} //empty list return
